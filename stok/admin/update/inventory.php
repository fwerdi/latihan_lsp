...
if ($id_gudang) {
    ...
    $stmt = $conn->prepare("SELECT nama_barang, jenis_barang, id FROM storage WHERE id_gudang = ?");
    

    $stmt->bind_param("i", $id_gudang);
    $stmt->execute();
    $stmt->bind_result($nama_barang, $jenis_barang, $id);
